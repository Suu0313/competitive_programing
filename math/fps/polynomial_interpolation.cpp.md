...
data:
  ...
  attributes:
    links: []
  bundledCode: "#line 2 \"math/fps/polynomial_interpolation.cpp\"\n\r\n#line 2 \"\
    math/fps/Formalpowerseries.cpp\"\n\r\n#line 2 \"math/fps/FFT-ntt.cpp\"\n\n#line\
    \ 2 \"math/mod/ModInt.cpp\"\n\r\ntemplate< int mod >\r\nstruct ModInt {\r\n  int\
    \ x;\r\n\r\n  constexpr ModInt() : x(0) {}\r\n\r\n  constexpr ModInt(int64_t y)\
    \ : x(y >= 0 ? y % mod : (mod - (-y) % mod) % mod) {}\r\n  \r\n  constexpr ModInt\
    \ &operator+=(const ModInt &p) {\r\n    if((x += p.x) >= mod) x -= mod;\r\n  \
    \  return *this;\r\n  }\r\n  constexpr ModInt &operator-=(const ModInt &p) {\r\
    \n    if((x += mod - p.x) >= mod) x -= mod;\r\n    return *this;\r\n  }\r\n  constexpr\
    \ ModInt &operator*=(const ModInt &p) {\r\n    x = (int) (1LL * x * p.x % mod);\r\
    ...
    \n    }\r\n    return res.pre(d);\r\n  }\r\n};\r\n#line 4 \"math/fps/polynomial_interpolation.cpp\"\
    \n\r\n#line 2 \"math/fps/multipoint_evaluation.cpp\"\n\r\n#line 4 \"math/fps/multipoint_evaluation.cpp\"\
    \n\r\ntemplate< typename T >\r\nvector<T> multipoint_evaluation(const Formalpowerseries<T>\
    \ &f, const vector<T> &xs){\r\n  int n0 = int(xs.size()), n = 1;\r\n  while(n\
    \ < n0) n <<= 1;\r\n  vector<Formalpowerseries<T>> g(n*2, {1});\r\n  for(int i\
    \ = 0; i < n0; ++i) g[i+n] = {-xs[i], 1};\r\n  for(int i = n-1; i > 0; --i) g[i]\
    \ = g[i*2] * g[i*2 + 1];\r\n\r\n  g[1] = f % g[1];\r\n  for(int i = 2; i < n +\
    \ n0; ++i) g[i] = g[i>>1] % g[i];\r\n  vector<T> res(n0);\r\n  for(int i = 0;\
    \ i < n0; ++i) res[i] = g[i+n][0];\r\n  return res;\r\n}\r\n#line 6 \"math/fps/polynomial_interpolation.cpp\"\
    \n\r\n\r\ntemplate<typename T>\r\nFormalpowerseries<T> polynomial_interpolation(const\
    \ vector<T> &xs, const vector<T> &ys){\r\n  using fps = Formalpowerseries<T>;\r\
    \n  int n = int(xs.size());\r\n  \r\n  queue<fps> fs;\r\n  for(auto&&x : xs) fs.push({-x,\
    \ 1});\r\n  while(int(fs.size()) > 1){\r\n    fps f = fs.front(); fs.pop();\r\n\
    \    fs.push(f * fs.front()); fs.pop();\r\n  }\r\n  fps g = fs.front();\r\n  vector<T>\
    \ dys = multipoint_evaluation(g.diff(), xs);\r\n  \r\n  queue<pair<fps, fps>>\
    \ qu;\r\n  for(int i = 0; i < n; ++i){\r\n    qu.emplace(fps{ys[i] / dys[i]},\
    \ fps{-xs[i], 1});\r\n  }\r\n  while(int(qu.size()) > 1){\r\n    auto[f1, g1]\
  ...
  code: "#pragma once\r\n\r\n#include \"./Formalpowerseries.cpp\"\r\n\r\n#include\
    ...
    \ polynomial_interpolation(const vector<T> &xs, const vector<T> &ys){\r\n  using\
    \ fps = Formalpowerseries<T>;\r\n  int n = int(xs.size());\r\n  \r\n  queue<fps>\
    \ fs;\r\n  for(auto&&x : xs) fs.push({-x, 1});\r\n  while(int(fs.size()) > 1){\r\
    \n    fps f = fs.front(); fs.pop();\r\n    fs.push(f * fs.front()); fs.pop();\r\
    \n  }\r\n  fps g = fs.front();\r\n  vector<T> dys = multipoint_evaluation(g.diff(),\
  ...
  verifiedWith: []
documentation_of: math/fps/polynomial_interpolation.cpp
layout: document
redirect_from:
- /library/math/fps/polynomial_interpolation.cpp
- /library/math/fps/polynomial_interpolation.cpp.html
